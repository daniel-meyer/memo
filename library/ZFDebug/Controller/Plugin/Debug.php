...
class ZFDebug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    ...
    protected function _headerOutput() {
        ...
        return ('
            <style type="text/css" media="screen">
                ...
                .ZFDebug_span { margin:0; border: 1px solid #999; border-right:0px; background:#DFDFDF; padding: 5px 5px; }
            ...
            <script type="text/javascript" charset="utf-8">
                ...
                var ZFDebug_toggler = null;
                var ZFDebug_debug = null;
                
                function ZFDebugCollapsed() {
                    if ('.$collapsed.' == 1) {
                        ZFDebugPanel();
                        ZFDebug_toggler.innerHTML="&#187;";
                        var size_d = GetElementOuterSize(ZFDebug_debug);
                        var size_t = GetElementOuterSize(ZFDebug_toggler);
                        
                        return ZFDebug_debug.style.left="-"+parseInt(size_d.width-size_t.width+1)+"px";
                    }
                }
                
                function ZFDebugPanel(name) {
                    ZFDebug_toggler = document.getElementById("ZFDebug_toggler");
                    ZFDebug_debug = document.getElementById("ZFDebug_debug");
                    
                    var allHTMLTags=document.getElementsByTagName("*");
                    for (var i=0; i<allHTMLTags.length; i++) {
                        if (allHTMLTags[i].className=="ZFDebug_panel") {
                        
                            if(allHTMLTags[i].style.display == "block") {
                                allHTMLTags[i].style.display = "none";
                            } else {
                                if (allHTMLTags[i].id == name)
                ...
                function ZFDebugSlideBar() {
                    ...
                    if (ZFDebug_debug.offsetLeft >  0) {
                        document.cookie = "ZFDebugCollapsed=1;expires=;path=/";
                        ZFDebugPanel();
                        ZFDebug_toggler.innerHTML = "&#187;";
                        var size_d = GetElementOuterSize(ZFDebug_debug);
                        var size_t = GetElementOuterSize(ZFDebug_toggler);
                        ZFDebug_debug.style.left="-"+parseInt(size_d.width-size_t.width+1)+"px";
                    } else {
                        document.cookie = "ZFDebugCollapsed=0;expires=;path=/";
                        ZFDebug_toggler.innerHTML = "&#171;";
                        ZFDebug_debug.style.left = "5px";
                    }
                }

                function ZFDebugToggleElement(name, whenHidden, whenVisible){
                    if(document.getElementById(name).style.display =="none"){
                        document.getElementById(whenVisible).style.display = "block";
                        document.getElementById(whenHidden).style.display = "none";
                        document.getElementById(name).style.display = "block";
                    } else {
                        document.getElementById(whenVisible).style.display = "none";
                        document.getElementById(whenHidden).style.display = "block";
        ...
        function GetElementOuterSize (element) { 
            if (element.offsetHeight) {
                var totalW = element.offsetWidth;
                var totalH = element.offsetHeight;
            } else {
                if (element.clientWidth){
                    ...
                    var frameW = clientW - element.clientWidth;
                    var frameH = clientH - element.clientHeight;
                    ...
                    if (clientW != element.clientWidth || clientH != element.clientHeight) {
                            // repair the mistake
                        frameW = totalW - element.clientWidth;
                        frameH = totalH - element.clientHeight;
    ...
    protected function _headerOutputJQuery() {
        $collapsed = isset($_COOKIE['ZFDebugCollapsed']) ? $_COOKIE['ZFDebugCollapsed'] : 0;
    ...
    protected function _output($html)
    {
        ...
        $content = $this->_headerOutput() . $html;
        $response->setBody(str_ireplace('</body>', '<div id="ZFDebug_debug">'.$content.'</div></body>', $response->getBody()));
    }
}