...
class Zsamer_Db_Table_Row_Orm extends Zend_Db_Table_Row_Abstract
{
    ...
    protected function _cleanCache()
    {
        ...
        $cache->clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, array($this->getTableClass()));
    }
	 ...
	 * @param array $data
	 * @return Zsamer_Db_Table_Row_Orm
	 */
	public function populate($data = array()) {
		...
		if (!is_array($data)) {
			throw new Zend_Db_Table_Row_Exception('Data must be array or object');
		}
		
		foreach ($data as $key => $value) {
		    $setMethod = 'set'.$this->_camelizeColumn($key);
            try{
    ...
    /**
	 * Sluggable Model with data
	 * @throws Zend_Db_Table_Row_Exception
	 * @param string $fromColumnName
	 * @return Zsamer_Db_Table_Row_Orm
	 */
    public function slug($fromColumnName, $prefix=''){
        $filter = new Etd_Filter_Slug;
        $value = $filter->filter($this->_getData($fromColumnName));
        
        $value = $this->_findEmptySlug('slug', $prefix . $value);
        $this->setSlug($value);
        
        return $this;
    }
    
    private function _findEmptySlug($slugColumnName, $value, $iteration=0)
    {
        $tryValue = $value;
        if(0 != $iteration){
            $tryValue = $value . '-' . $iteration;
        }
        
        $row = $this->_getTable()->fetchRow(array($slugColumnName."=?"=>$tryValue, 'id!=?'=>(int)$this->getId()));
        if(null != $row){
            return $this->_findEmptySlug($slugColumnName, $value, $iteration+1);
        }
        return $tryValue;
    }
	...
	protected function _setupDependentTables()
	{
		...
		foreach($dependentTables as $table)
		{
            ...
            $columnName = $this->_underscore($chieldClassName);
			...
			} else if(isset($this->_dataObjectCollection[$columnName])){
				$objectCollection = $this->_dataObjectCollection[$columnName];
	...
	public function setData($key, $value='')
	{
		...
		if (array_key_exists($columnName, $this->_data)) {
    		...
    		//date column
            if($this->_isDateColumn($columnName)){
                if($value==''){
                    $value = null;
                }
		...
		if ($value === null)
		{
            try{
                ...
                if (array_key_exists($columnName, $this->_dataObject)) {
                    unset($this->_dataObject[$columnName]);
                }
                $referenceMap = $this->_getTable()->getReferenceMapOrm();
                $info = $referenceMap[$this->_camelizeColumn($columnName)];
                $setParentKeyMethod = 'set'.$this->_camelizeColumn(current($info['columns']));

			    $this->$setParentKeyMethod(null);
    ...
    protected function _getPrimaryVal()
    {
        $primary = (array)$this->_primary;
        $out = array();
        foreach($primary as $columnName){
            $out[] = $this->_data[$columnName];
        }
        return join('-', $out);
    }
    
    protected function _prepareOutputData($columnName)
    {
        $metadata = $this->getTable()->info('metadata');
        
        if($this->_isDateColumn($columnName) && is_string($this->_data[$columnName])){
            return new DateTime($this->_data[$columnName]);
        }
	...
	public function getData($columnName = null, Zend_Db_Table_Select $select = null)
	{
		...
		if (array_key_exists($columnName, $this->_data)) {
			return $this->_prepareOutputData($columnName);
		}
        ...
        $cacheName = $columnName . '#' . $this->_getPrimaryVal();
		...
		if (isset(self::$_objectPersistentCache[$cacheName]) && $this->_equalsCriteria($columnNameCriteria, $select)) {
			return self::$_objectPersistentCache[$cacheName];
		}

		$dependentTables = $this->_getTable()->getDependentTables();
        
        // find in dependent Tables
		foreach($dependentTables as $table)
		{
			$chieldClassName = $this->_getTableClassName($table);
			$dependentTable = $this->_underscore($chieldClassName);

			if ($dependentTable === $columnName) {
				if($isCollection === false){
					self::$_objectPersistentCache[$cacheName] = $this->findDependentRowset($table, null, $select)->current();
				} else{
					self::$_objectPersistentCache[$cacheName] = $this->findDependentRowset($table, null, $select);
				}
        ...
        // find in parents Tables
		$referenceMap = $this->_getTable()->getReferenceMapOrm();
		foreach($referenceMap as $map => $info)
		{
			if ($this->_underscore($map) === $columnName) {
				$table = $info['refTableClass'];
				self::$_objectPersistentCache[$cacheName] = $this->findParentRow($table, null, $select);
				self::$_objectPersistentCache[$columnNameCriteria] = $select;
				return self::$_objectPersistentCache[$cacheName];
				break;
			}
        ...
        if($isCollection === true){
    		...
    		{
                $t = new $table;
                $referenceMap = $t->getReferenceMapOrm();
                // manyToMany table has always 2 primary keys
                if(2 == count($t->info('primary'))){
                    foreach($referenceMap as $map => $info)
            		{
            			if ($this->_underscore($map) === $columnName) {
            				$table2 = $info['refTableClass'];
            				self::$_objectPersistentCache[$cacheName] = $this->findManyToManyRowset($table2, $table, null, null, $select);
            				self::$_objectPersistentCache[$columnNameCriteria] = $select;
            				return self::$_objectPersistentCache[$cacheName];
            				break;
            			}
	...
	protected function _isDateColumn($columnName)
	{
        $metadata = $this->getTable()->info('metadata');
        return in_array($metadata[$columnName]['DATA_TYPE'], array('datetime', 'timestamp', 'date'));
	}
	...
	protected function _underscore($name)
	{
        ...
        $result = strtolower($filter->filter($name));
